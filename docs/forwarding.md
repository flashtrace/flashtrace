...
- Forwarding chains must be acyclic; a forwarding of an item to itself counts as a cycle too. Every forwarding on a cycle is flagged as a *cyclic* defect on its source item and has no effect - the items on the cycle fall back to their own `Needs`.
- A forwarding target counts as demanded coverage: a code item referenced only as a forwarding target is not *unwanted*. Only a valid forwarding demands its target: one voided as invalid does not, so a code item referenced only by an invalid forwarding is still flagged *unwanted* - in addition to the *cyclic* defect on each source of a cyclic chain, or to the *problem* for a forwarding from a non-existent source.
