...
- **Keywords** - after the description, an item may carry:
  - `Needs:` - the IDs of the items required to cover this one. Because needs are explicit IDs, multiple needed items of the same type are possible. A need may use a [wildcard revision](revisions.md#wildcard-revisions) (e.g. `impl:auth/login#2.x`) to accept any matching downstream revision. A need may also use the short form `<type>#<revision>` (e.g. `impl#1`): it names the item of that type with exactly the `[group/[group/]]name` of the item specifying the need, so inside `req:auth/login#1` the entry `impl#1` stands for `impl:auth/login#1`. Wildcard revisions work in the short form too (`impl#2.x`); a bare type without `#<revision>` is not a need entry.
