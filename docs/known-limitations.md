...
- Paths are compared lexically and exactly. On a case-insensitive filesystem (Windows, macOS by default), an input path spelled in a different case than the working directory is reported by its absolute path instead of a relative one.
