...
The need target of either form may use a [wildcard revision](revisions.md#wildcard-revisions) (e.g. `[>>utest:auth/login#2.x]`) or the short form `<type>#<revision>` (e.g. `[>>utest#1]`), which names the item of that type with exactly the `[group/…]name` of the item the need attaches to - after `[impl:auth/login#1]`, the tag `[>>utest#1]` demands `utest:auth/login#1`. The item tag and the `<source-id>` anchor must name a full ID with a concrete revision.
